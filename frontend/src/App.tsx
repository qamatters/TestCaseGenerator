...
const emptyCase = (): TestCase => ({ id: crypto.randomUUID(), selected: false, testName: '', objective: '', preCondition: '', steps: '', testData: '', expectedResult: '', priority: 'Medium' });
...
export default function App() {
  ...
  return <div className='page'><h1>QA Test Case Generator</h1>
    <div className='grid'>
      <input placeholder='Jira Story ID' value={jiraStoryId} onChange={e => setJiraStoryId(e.target.value)} />
      <button onClick={async () => setJiraDetails(await api.fetchJira(jiraStoryId))}>Fetch Jira details</button>
      <select value={project} onChange={e => setProject(e.target.value)}><option value=''>Project Name</option><option>QAT</option></select>
      <select value={folder} onChange={e => setFolder(e.target.value)}><option value=''>Folder</option><option>Regression/API</option></select>
    ...
    <div className='toolbar'>
      <button onClick={async () => alert(await api.review(jiraDetails))}>Get AI review</button>
      <button onClick={onGenerate}>Generate test cases</button>
      <button onClick={() => window.open('', '_blank')?.document.write(`<pre>${JSON.stringify(testCases, null, 2)}</pre>`)}>HTMLPreview</button>
      <button onClick={() => {setJiraStoryId('');setJiraDetails('');setAdditional('');setTestCases([]);}}>Clear all</button>
      <button onClick={() => setTestCases([])}>Reset Test Cases</button>
      <button onClick={() => alert('Use Fetch Jira details, enrich content, pick prompt + model, then generate and publish.')}>Help</button>
    ...
    <div><button onClick={() => alert(`Publishing ${testCases.filter(t=>t.selected).length} selected test cases...`)}>Publish selected</button><button onClick={() => alert(`Publishing ${testCases.length} test cases to story...`)}>Bulk publish to story</button></div>
  </div>;
}
